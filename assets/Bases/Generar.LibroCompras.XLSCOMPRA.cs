...
namespace Softech.Administrativo.Generacion
{
    ...
    public static class XLSCOM
    {
        ...
        private static Boolean imprimirColumnasImport;
        ...
        public static void Generar(DataSet datos, Dictionary<String, Object> filtros, Dictionary<String, Object> parametros, String ruta, String nombreArchivo)
        {
            ...
            if (!ds.Tables.Contains("XLSCOMPRA"))
                throw new Exception("No existe la tabla XLSCOMPRA en el DataSet.");

            imprimirColumnasImport = (filtros["f_filtro3"] == "NO" || Equals(filtros["f_filtro3"], null))
                                        ? false
                                        : true;
        ...
        private static void imprimir(DataSet dt, String ruta, String nombreArchivo)
        {
            ...
            #region crear xlsx desde cero
            //documento = SpreadsheetDocument.Create(ruta + nombreArchivo, SpreadsheetDocumentType.Workbook, false);
            ////Se crean los archivos del xlsx
            ////workbook.xml
            //documento.AddWorkbookPart();
            //documento.WorkbookPart.Workbook = new Workbook();
            //documento.WorkbookPart.Workbook.Save();

            ////sharedStrings.xml
            //SharedStringTablePart sharedStringTablePart;
            //sharedStringTablePart = documento.WorkbookPart.AddNewPart<SharedStringTablePart>();
            //sharedStringTablePart.SharedStringTable = new SharedStringTable();
            //sharedStringTablePart.SharedStringTable.Save();

            ////Carpeta worksheets y sheet.xml
            //documento.WorkbookPart.Workbook.Sheets = new Sheets();
            //documento.WorkbookPart.Workbook.Save();

            ////styles.xml
            //WorkbookStylesPart workbookStylesPart;
            //workbookStylesPart = documento.WorkbookPart.AddNewPart<WorkbookStylesPart>();
            //workbookStylesPart.Stylesheet = new Stylesheet();
            //workbookStylesPart.Stylesheet.Save();

            //Stylesheet stylesheet = documento.WorkbookPart.WorkbookStylesPart.Stylesheet;

            //stylesheet.InsertAt<CellFormats>(new CellFormats(), 0);
            ...
            //Sheets sheets = documento.WorkbookPart.Workbook.GetFirstChild<Sheets>();
            //Sheet sheet;
            //WorksheetPart worksheetPart;

            ////Se crea la nueva hoja sheet.xml
            //worksheetPart = documento.WorkbookPart.AddNewPart<WorksheetPart>();
            //worksheetPart.Worksheet = new Worksheet(new SheetData());
            //worksheetPart.Worksheet.Save();

            ////Se vincula la hoja con el libro
            //sheet = new DocumentFormat.OpenXml.Spreadsheet.Sheet()
            //{
            //    Id = documento.WorkbookPart.GetIdOfPart(worksheetPart),
            //    SheetId = (uint)(documento.WorkbookPart.Workbook.Sheets.Count() + 1),
            //    Name = "Hoja1"
            //};
            //sheets.Append(sheet);
            //documento.WorkbookPart.Workbook.Save();

            //SharedStringTable sharedStringTable = documento.WorkbookPart.SharedStringTablePart.SharedStringTable;

            //if (0 == sharedStringTable.Where(item => item.InnerText == "").Count())
            //{
            //    sharedStringTable.AppendChild(
            //       new DocumentFormat.OpenXml.Spreadsheet.SharedStringItem(
            //          new DocumentFormat.OpenXml.Spreadsheet.Text("")));
            //}
            ////Guardo
            ...
            String _Plantilla = /*imprimirColumnasImport
                            ? "Plantilla_ImportXLSCOMPRA.xlsx"
                            : */"PlantillaXLSCOMPRA.xlsx";
        ...
        {
            DataRow r = dt.Tables["XLSCOMPRA"].Rows[0];
        ...
        private static SpreadsheetDocument Imprimir_02_01_encabezado_0(DataSet dt, String ruta, String nombreArchivo, SpreadsheetDocument documento)
        {
            ...
            documento = UpdateValue("S" + filaTitulos.ToString(), "I.V.A. Retenido (al vendedor)", 7, CellValues.String, documento);
            documento = UpdateValue("T" + filaTitulos.ToString(), "I.V.A. Retenido (a terceros)", 7, CellValues.String, documento);
            documento = UpdateValue("U" + filaTitulos.ToString(), "Anticipo de I.V.A. (importación)", 7, CellValues.String, documento);
        ...
        {
            for (int i = 0; i < dt.Tables["XLSCOMPRA"].Rows.Count; i++)
            {
                DataRow r = dt.Tables["XLSCOMPRA"].Rows[i];
